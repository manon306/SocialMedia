...
public class ChatHub : Hub
{
    ...
    {
        var imagePath = Context.User?.Claims.FirstOrDefault(c => c.Type == "ImagePath")?.Value; // نجيب الصورة من الـ Claims
        var userName = Context.User?.Identity?.Name; // نجيب اسم اللي عامل Login
        await Clients.All.SendAsync("ReceiveMessage", userName, imagePath, message);
    }
}